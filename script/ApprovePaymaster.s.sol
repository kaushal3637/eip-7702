...
contract ApprovePaymaster is Script {
    function run() external {
        ...
        address paymasterAddress = vm.envOr(
            "USDC_PAYMASTER",
            address(0x8254B120B4839F847dFAc558c64f0DC9A1B5173E)
        );
        ...
        if (walletAddress.code.length == 0) {
            ...
            console.log(
                "Please run CreateWallet script first or check the wallet address."
            );
            return;
        }
        ...
        try wallet.owner() returns (address walletOwner) {
            ...
            console.log(
                "ERROR: Cannot verify wallet owner. Wallet may not be properly initialized."
            );
            return;
        }
        ...
        if (currentAllowance >= approvalAmount) {
            ...
            uint256 newAllowance = usdc.allowance(
                walletAddress,
                paymasterAddress
            );
            ...
            if (newAllowance >= approvalAmount) {
                console.log(
                    " Approval successful! Paymaster can now spend USDC for gas fees."
                );
            } else {
